...
import { AnswerSelectorComponent, OptionAnswerSelector } from 'src/app/shared/components/answer-selector/answer-selector.component';
...
export class CalendarCreatorComponent implements OnInit {
  currentStep: 'MAIN' | 'RESERVATION_PARAMS' | 'RESERVATION_AVAILABILITY' = 'MAIN';
  user: User = null;
  ...
  reservationTimeAvailability: OptionAnswerSelector[] = [
    {
      value: 'Ingresa el rango de tiempo en el cuál se podrán hacer reservaciones',
      status: true,
      icons: [
        {
          src: 'https://storage-rewardcharly.sfo2.digitaloceanspaces.com/new-assets/pencil.svg',
  ...
  calendarCreatorForm: FormGroup = new FormGroup({
    reservationParams: new FormGroup({
      amountOfReservationsAtTheSameTime: new FormControl(null, Validators.compose(
        [Validators.required, Validators.min(1)]
      )),
      reservationDurationInMinutes: new FormControl(null, Validators.compose(
  ...
  async ngOnInit() {
    ...
    for(let number = 0; number <= 23; number++) {
      this.hours.push(
        number.toString().length < 2 ? '0' + number :
        number
      );
    }
  ...
  changeStepTo(step: 'MAIN' | 'RESERVATION_PARAMS' | 'RESERVATION_AVAILABILITY') {
    this.currentStep = step;
  }

  selectDayOfTheWeek(dayIndex: number) {
    this.daysOfTheWeek[dayIndex].selected = !this.daysOfTheWeek[dayIndex].selected;
    const reservationAvailabilityFormGroup = this.calendarCreatorForm.controls.reservationAvailability as FormGroup;
    ...
    if(this.daysOfTheWeek[dayIndex].selected) {
      ...
      const clone = reservationAvailabilityFormGroup.controls.daysAvailability.value;
      clone.sort((a, b) => {
        return a.order - b.order;
      });

      reservationAvailabilityFormGroup.controls.daysAvailability.setValue(clone);
    } else {
      reservationAvailabilityFormGroup.controls.daysAvailability.value.filter((day) => {
  ...
  selectHour(type: string, index: number, hour: number) {
    ...
    if(this.selectedFromHour?.hour && this.selectedFromMinutes?.minute) {
      reservationAvailabilityFormGroup.controls.fromHour.setValue(`${this.selectedFromHour.hour}:${this.selectedFromMinutes.minute}`);
    }

    if(this.selectedToHour?.hour && this.selectedToMinutes?.minute) {
      reservationAvailabilityFormGroup.controls.toHour.setValue(`${this.selectedToHour.hour}:${this.selectedToMinutes.minute}`);
    }
  } 
  ...
  async save() {
    ...
    switch(this.currentStep) {
      ...
      case 'MAIN':
        ...
        */
        const daysInOrder = daysAvailability.map(dayObject => dayObject.fullname);
        ...
        const result = await this.calendarService.createCalendar({
          name: "Calendario genérico " +  Math.floor(Math.random() * (100 - 1)) + 1,
      ...
      case 'RESERVATION_PARAMS':
        if(
          ...
          this.reservationDurationAndCounterCTA[0].value = `${
            amountOfReservationsAtTheSameTime
          } reservas cada ${reservationDurationInMinutes} min`
      ...
      case 'RESERVATION_AVAILABILITY':
        if(
          ...
          if(fromHourNumber > 12) fromHourFinal = `${fromHourNumber - 12}:${fromHour.split(":")[1]} PM`;
          else fromHourFinal = `${fromHourNumber}:${fromHour.split(":")[1]} AM`

          if(toHourNumber > 12) toHourFinal = `${toHourNumber - 12}:${toHour.split(":")[1]} PM`;
