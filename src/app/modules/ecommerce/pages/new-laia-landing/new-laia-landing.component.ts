...
export class NewLaiaLandingComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.translate
      .get('modal.convertAudioText')
      .subscribe((translate) => (this.convertAudioText = translate));
  }
  ...
  async saveAudio(chatText: boolean = false) {
    ...
    try {
      ...
      const result = await this.gptService.openAiWhisper(
        this.audio &&
          new File([this.audio.blob], this.audio.title || 'audio.mp3', {
            type: (<Blob>this.audio.blob)?.type,
          })
      );
  ...
  resizeTextarea(textarea) {
    ...
    if (textarea.scrollHeight > textarea.clientHeight) {
      textarea.style.height =
        textarea.scrollHeight > 46 ? textarea.scrollHeight + 'px' : 46 + 'px';
    } else {
  ...
  public async install(): Promise<void> {
    if (this._deferredPrompt) {
      ...
      if (outcome === 'accepted') {
        ...
        console.log('Instalación de la aplicación rechazada');
      }
