...
export class AuthClassicComponent implements OnInit {
  ...
  async ngOnInit() {
    this.route.queryParams.subscribe(async (params) => {
      ...
      await this.getExchangeData(
        this.header.saleflow.module.paymentMethod.paymentModule._id
      );
  ...
  createOrSkipOrder() {
    if (this.orderId) {
      this.router.navigate(['ecommerce/payments']);
    } else {
      lockUI(
        this.createOrder().then((data) => {
          return this.getOrderData(this.header.orderId).then(() => {
            this.router.navigate([`ecommerce/payments/${data}`]);
          });
        })
      );
    }
