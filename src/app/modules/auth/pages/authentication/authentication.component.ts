...
import {
  MultistepFormServiceService,
  MultistepFormStorage,
} from 'src/app/core/services/multistep-form-service.service';
  ...
  formSteps: FormStep[] = [
    {
      fieldsList: [
        {
          ...
          label:
            '¿En cuál o cuáles # de WhatsApp recibirás las notificaciones de las ordenes?',
          inputType: 'phone',
          styles: {
            containerStyles: {
              marginTop: '38px',
      ...
      statusChangeCallbackFunction: (change) => {
        ...
        else this.formSteps[0].fieldsList[1].disabled = true;
      },
      ...
      this.formSteps[0].asyncStepProcessingFunction = {
        ...
        function: async (params) => {
          ...
          if (!isMerchant) {
            ...
            if (!foundUser) {
              const socialsFiltered = Object.keys(socials)
                .filter((socialNetworkKey) => {
                  return socials[socialNetworkKey] &&
                    socials[socialNetworkKey].length > 0
                    ? true
                    : false;
                })
              ...
              const createdUser = await this.authService.signup(
                ...
                true,
                userImage
                  ? checkIfStringIsBase64DataURI(userImage)
                    ? base64ToFile(userImage)
                    : null
                  : null
              );
              ...
              const magicLinkCreated = await this.authService.generateMagicLink(
                ...
                userImageConverted ? [userImageConverted] : null
              );
          ...
          if (isMerchant) {
            ...
            if (!foundUser) {
              ...
              if (!merchantImageConverted)
                createMerchantInput.image =
              ...
              const defaultMerchant =
                await this.merchantService.merchantDefault();

              const merchantImageConverted =
                userImage.length > 0
                  ? checkIfStringIsBase64DataURI(userImage)
                    ? base64ToFile(userImage)
                    : null
              ...
              if (!defaultMerchant) {
                ...
                const magicLinkCreated =
                  await this.authService.generateMagicLink(
                    ...
                    merchantImageConverted ? [merchantImageConverted] : null
                  );
