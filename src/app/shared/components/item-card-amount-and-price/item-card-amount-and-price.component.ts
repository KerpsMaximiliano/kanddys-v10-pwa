...
export class ItemCardAmountAndPriceComponent implements OnInit {
    ...
    if (this.itemData.customizerId) {
      if (bool)
        this.header.order.products[0].amount +=
          this.itemData.qualityQuantity.quantity;
      else if (
        this.header.order.products[0].amount !==
        this.itemData.qualityQuantity.quantity
      )
      ...
      const total =
        this.itemData.qualityQuantity.price +
        this.header.order.products[0].amount *
      ...
      if (this.header.order.products[0].amount > 0) {
        this.header.order.products[0].amount =
          this.header.order.products[0].amount + 1;
  ...
  onAnimStart(event: any) {
    if (event.toState === 'small' && event.phaseName === 'done')
